...
package. Their recommended order, and the two traps in it — a panicking request is not logged
...
`CORSConfig` is everything `CORS` needs. `CORS` copies the origins and compiles the rest into fixed
header values when it is called, and never reads the struct again. `Origins` is the one required
