...
earlier offer; no header, or only blank `Accept` lines, gives the first offer. An offer whose type
or subtype is not an RFC 9110 token, or is `*`, panics with the `rice: ` prefix, checked even when
there is no `Accept` header so a bad offer fails on the first request rather than the first one that
sends the header. Quality is parsed straight into thousandths — no float, no `strconv` — by
`parseQ`, which accepts RFC 9110's `qvalue` grammar and, for Java, a leading-dot `q=.2`. `Accepts`
adds `Vary: Accept` with `Add`, once, whatever it returns and whether or not the request sent
`Accept`, unless a `Vary` line already lists `Accept` in any case or `*`; it adds rather than sets,
so a `Vary: Origin` a handler or CORS wrote first survives. It writes no status: a handler with
nothing acceptable returns `ErrNotAcceptable`, answered 406 through the same funnel as every other
error. It stores nothing on `Ctx` and allocates nothing. `ctx_accept_test.go`'s tests group into the
matching table (`TestAcceptsMatchesByRFC9110`, 43 cases including curl's `*/*`, Java's default
header, a wildcard `q=0`, and a 50-range header where only the last element matters), `parseQ`
(`TestParseQ`), panics (`TestAcceptsPanicsOnABadOffer`,
`TestAcceptsValidatesOffersWithoutAnAcceptHeader`), `Vary` (`TestAcceptsAddsVaryAccept`,
`TestAcceptsAddsVaryWithoutAnAcceptHeader`, `TestAcceptsKeepsAVaryTheHandlerSet`,
`TestA406KeepsVaryAccept`), the funnel (`TestErrNotAcceptableAnswers406`), and a fuzz target
(`FuzzAccepts`). Four budgets — `TestAllocBudgetAccepts`, `TestAllocBudgetAcceptsNoHeader`,
`TestAllocBudgetAcceptsTwice`, `TestAllocBudgetNotAcceptable` — and a benchmark,
`BenchmarkCtxAccepts`, negotiating two offers against Chrome's eight-element navigation `Accept`
header. Fixing `TestEveryCtxMethodPanicsAfterRelease` for a variadic method changed
`ricedebug_test.go`'s walker to call `CallSlice` instead of `Call` when a method's last parameter is
variadic. [ADR-0018](adr/0018-accepts-negotiates-by-q-and-adds-vary.md) records the decision;
`03-core-concepts.md` gets the `Accepts` row in §2's read-side table, its own paragraph, and a
sentence on `ErrNotAcceptable` in §7; `05-performance-model.md` pins the four budgets and the
benchmark in the `Ctx` methods and dispatch tables; `04-roadmap.md` moves the item from *Explicitly
deferred* to *Done after M8*; the ADR index gets a row.

**Learned:** Four things, plus one surprise.
...
4. Java's `HttpURLConnection` sends `Accept: text/html, image/gif, image/jpeg, *; q=.2, */*; q=.2`
   by default. `q=.2` is not an RFC 9110 `qvalue`, so a strict parser drops the `*/*` element and
   refuses a JSON-only handler; the final review found it, and `parseQ` now accepts `.` followed by
   one to three digits.
5. Surprise met writing this documentation: `make cover`'s root-package figure was 99.3%, not the
   99.5% this plan's brief expected, from gaps in `ctx_accept.go` at `trimOWSString` and
   `matchRange`. The final review's table rows closed them: every function in `ctx_accept.go` is
   now at 100%.
...
without `-race`. `BenchmarkCtxAccepts`: median 520.9 ns/op, 0 B/op, 0 allocs/op over ten runs, on
darwin/arm64 (Apple M2 Pro, go1.25.6) — a full dispatch negotiating two offers against Chrome's
eight-element header and adding `Vary`; a CPU profile put the `Accept` scanning (`offerQuality`,
through `nextItem` and `matchRange`) at about 73% of that time and adding `Vary` at about 9%. No
results file was committed for it; the figure is cited here and in `05-performance-model.md` in
prose instead. `FuzzAccepts` ran 60 s and found no failure over 6,782,845 executions. `make cover`:
root package 99.6% (99.5% total across modules), every function in `ctx_accept.go` at 100%.
...
a directory on disk, `embed.FS` (through `fs.Sub`) for files compiled in — for GET and HEAD.
`Static` registers three patterns per call: the bare prefix, redirected to its slash;
`prefix+"/"`, the fs root; and `prefix+"/*filepath"`, everything beneath it. Each call records a
`staticEntry`; `Build` constructs its one long-lived `fasthttp.FS` and calls `NewRequestHandler`
...
`TestGroupStaticJoinsThePrefixAndRunsTheGroupMiddleware`, `TestStaticRouteMiddlewareRuns`,
`TestARegisteredRouteOutranksAStaticFile`, `TestStaticPanicsOnBadConfiguration`,
`TestStaticDispatchedBeforeBuildIsAnError`), funnel (`TestStaticFailuresReachTheErrorHandler`,
...
1. *fasthttp's directory redirect is built from the rewritten path, not the request's.* Probing
   ...
   because fasthttp appends `/` to the path `PathRewrite` handed it, not to the request's own.
   rice replaces the status and `Location` itself rather than trusting fasthttp's.

2. *`Root: "."` breaks the root index.* With `Root: "."`, fasthttp looks the root's `index.html`
   up as `./index.html`, which is not a valid `fs.FS` path, and answers 403. `Root` has to be
   `""`, with `AllowEmptyRoot: true`.

3. *`Compress` writes to disk.* Turning it on makes fasthttp create `<file>.fasthttp.gz` (and
   `.br`, `.zst`) next to the file through `os.CreateTemp`, relative to the process's working
   directory for an `fs.FS` with an empty root — a write nobody asked a static file server to
   make. `Compress` stays off.

4. *A `..` segment never reaches the handler's own check, because fasthttp already normalised it
   away.* `fctx.Path()` is normalised unconditionally before routing in fasthttp v1.73,
   `DisablePathNormalizing` or not, so a request carrying `..` is a route miss — a 404 — before
   `serve` runs; the `..` branch in `hasDotDotSegment` is a defensive second line, not the reason
   `..` is refused today. It surfaced reading fasthttp's routing, not from a failing test. A
   related surprise came from the lifecycle test: `TestStaticStartsNoGoroutineUntilBuild` flaked
   under `-race`, because fasthttp v1.73 stops a handler's cache goroutine only through a
   `runtime.AddCleanup`, which runs asynchronously after a GC — earlier tests' dropped,
   unreachable Apps were losing their goroutines at unpredictable moments and moving the baseline
   the test measured against. The fix, `stableCacheGoroutines`, turns automatic GC off for the
