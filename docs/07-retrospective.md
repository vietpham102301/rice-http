...
list — binding, timeouts, static files, content negotiation, streaming, `c.Query` and `c.Header`,
`c.JSON`, typed store keys — stays as it was: not promised, each needing its own design. If work resumes, the
